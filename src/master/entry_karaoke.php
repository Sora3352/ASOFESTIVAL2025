...
if (in_array($role, $can_delete_roles, true) && isset($_POST['delete_id'])) {
    ...
    $log_action = "カラオケ大会のエントリーID {$delete_id}（" . ($target['name'] ?? '不明') . "）を削除しました";
    ...
    header('Location: entry_karaoke.php');
    exit();
}
...
function canEdit($role, $user_project)
{
    return in_array($role, ['chief', 'vice', 'teacher', 'core'], true)
        || ($role === 'leader' && $user_project === 'karaoke');
}
...
// ===== アクセス制御 =====
if ($role === 'member' && $user_project !== 'karaoke') {
...
$sql_base = "FROM entries WHERE project = 'karaoke'";
$params = [];
...
<head>
    ...
    <title>カラオケ大会 | ASO FESTIVAL 2025 管理</title>
    <link rel="stylesheet" href="master.css">
</head>

<body>
    <header>カラオケ大会エントリー一覧</header>
    ...
    <div class="container">
        ...
        <form class="search-box" method="get" action="">
            ...
            <select name="filter_class">
                ...
                <?php
                $classList = $pdo->query("SELECT DISTINCT class FROM entries WHERE project='karaoke' ORDER BY class ASC")->fetchAll(PDO::FETCH_COLUMN);
